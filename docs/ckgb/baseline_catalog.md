...
| Product distribution/update architecture | Recommended for versioned desktop/local products | JAP proved stage-before-consent simplicity while PED proved publisher-signing value and the cost of local broker coupling | Local/desktop product with persistent releases, multiple target systems, or plausible future external users | Throwaway prototype or source-distributed developer tool with no install/update lifecycle |
...
- `docs/lessons-learned/product-update-architecture-ped-jap-2026-09.md` — PED/JAP updater, signing and distribution lessons plus the deliberate transitional PED decision.
...
- `docs/tooling/product-distribution-update-baseline.md` — project-start target for hosted build, centralized signing when selected, consumer-local verified staging, consent and isolated apply without CI-runner dependencies on consumer systems.
