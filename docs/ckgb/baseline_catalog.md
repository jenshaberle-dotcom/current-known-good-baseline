...
| [Lessons-learned glossary](../lessons-learned/glossary.md) | Recommended | Memorable project terms improved recognition but need durable technical definitions so metaphors do not become folklore or authority | Multi-project, agent-assisted, governance-heavy or long-lived work with reusable lessons | Tiny one-off work with no reusable vocabulary |
| [Temporal authority drift / Deadwalker detection](../controls/temporal_authority_drift_control.md) | Recommended | Historically valid but superseded rules can re-enter current reasoning through stale artifacts, context, templates, or semantic paraphrase | Long-lived, AI-assisted, governance-heavy, multi-session or evolving-authority projects | Short-lived work with no evolving rules, permissions or decisions |
